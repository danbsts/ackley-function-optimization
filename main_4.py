...
def ackley_function_optimization():
  ...
  while solution == None and count < iterations:
      ...
      solution = eval(population_fitness)
      if count % 100 == 0: print(count, '->', population_fitness[0][1], todo, '--', successful_mutations/total_mutations, 'dis->',calculate_mean(list(map(lambda x: x[0], population_fitness)), 1))
      count += 1
